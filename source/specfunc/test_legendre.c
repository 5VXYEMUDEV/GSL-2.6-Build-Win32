           ...
           const char *desc, const char *desc2)
{
  ...
  gsl_test_rel(value, expected, tol, "%s %s lmax="PCTZ"u l="PCTZ"u m="PCTZ"u", desc, desc2, lmax, l, m);
} /* test_value() */
      ...
      for (m = 0; m <= l; ++m)
        {
          ...
          gsl_test_rel(p[idx] / fac, p_expected[idx], 1.0e-10,
                       "%s %s l="PCTZ"u m="PCTZ"u", desc, desc2, l, m);
        }
    }
      ...
      for (l = 0; l <= lmax; ++l)
        {
          ...
          gsl_test_rel(sum, rhs, 1.0e-10,
                       "%s l="PCTZ"u, x=%f, sum=%.12e", desc, l, x, sum);
        }
    }
      ...
      for (i = 0; i < nlm; ++i)
        {
          ...
          /* check p = p_alt = p2 */
          gsl_test_rel(p[i], p2[i], 1.0e-10, "%s deriv i="PCTZ"u", desc, i);
          gsl_test_rel(p_alt[i], p2[i], 1.0e-10, "%s deriv_alt i="PCTZ"u", desc, i);

          /* check dp = -1/u*dp_alt */
          gsl_test_rel(-uinv * dp_alt[i], dp[i], 1.0e-10,
                       "%s deriv_alt x=%f i="PCTZ"u", desc, x, i);
        }
      ...
      for (l = 0; l <= lmax; ++l)
        {
          ...
          gsl_test_abs(sum, 0.0, 1.0e-10,
                       "%s deriv l="PCTZ"u, x=%f, sum=%.12e", desc, l, x, sum);
        }
    }
      ...
      for (i = 0; i < nlm; ++i)
        {
          if (fabs(p2[i]) < 1.0e3 * GSL_DBL_EPSILON)
            gsl_test_abs(p[i], p2[i], 1.0e-10, "%s deriv2 i="PCTZ"u", desc, i);
          else
            gsl_test_rel(p[i], p2[i], 1.0e-10, "%s deriv2 i="PCTZ"u", desc, i);
        }
      ...
      for (l = 0; l <= lmax; ++l)
        {
          ...
          gsl_test_abs(sum, 0.0, 1.0e-10,
                       "%s deriv2 l="PCTZ"u, x=%f, sum=%.12e", desc, l, x, sum);
          gsl_test_abs(sum2, 0.0, 1.0e-6,
                       "%s deriv2 l="PCTZ"u, x=%f, sum=%.12e", desc, l, x, sum2);
        }
    }
      ...
      for (l = 0; l <= (size_t) lmax; ++l)
        {
          ...
          gsl_test_rel(p[idx], p_schmidt[idx], 1.0e-10,
                       "legendre unnorm l="PCTZ"u, m=0, x=%f", l, x);
          gsl_test_rel(dp[idx], dp_schmidt[idx], 1.0e-10,
                       "legendre unnorm deriv l="PCTZ"u, m=0, x=%f", l, x);

          if (l > 1)
            {
              gsl_test_rel(d2p[idx], d2p_schmidt[idx], 1.0e-10,
                           "legendre unnorm deriv2 l="PCTZ"u, m=0, x=%f", l, x);
            }
          else
            {
          ...
          for (m = 1; m <= l; ++m)
            {
              ...
              gsl_test_rel(a_lm * p[idx], p_schmidt[idx], 1.0e-9,
                           "legendre unnorm l="PCTZ"u, m="PCTZ"u, x=%f", l, m, x);
              gsl_test_abs(a_lm * dp[idx], dp_schmidt[idx], 1.0e-10,
                           "legendre unnorm deriv l="PCTZ"u, m="PCTZ"u, x=%f", l, m, x);
              gsl_test_abs(a_lm * d2p[idx], d2p_schmidt[idx], 1.0e-10,
                           "legendre unnorm deriv2 l="PCTZ"u, m="PCTZ"u, x=%f", l, m, x);
          ...
          for (m = 0; m <= l; ++m)
            {
              ...
              gsl_test_rel(p2[idx], p[idx], 1.0e-10,
                           "%s compare l="PCTZ"u, m="PCTZ"u, x=%f",
                           desc, l, m, x);
            }
        }
    }
