...
test_poisson(const size_t N, const double epsrel, const int compress)
{
  ...
  gsl_test(status, "%s poisson status s=%d N="PCTZ"u", desc, status, N);
  ...
  for (i = 0; i < n; ++i)
    {
      ...
      gsl_test_rel(u_gsl, u_exact, epsrel, "%s poisson N="PCTZ"u i="PCTZ"u",
                   desc, N, i);
    }
  ...
  /* check that the residual satisfies ||r|| <= tol*||b|| */
  {
    ...
    gsl_test(status, "%s poisson residual N="PCTZ"u normr=%.12e normb=%.12e",
             desc, N, normr, normb);
              ...
              const double c)
{
  ...
  gsl_test(status, "%s toeplitz status s=%d N="PCTZ"u a=%f b=%f c=%f",
           desc, status, N, a, b, c);
  ...
  /* check that the residual satisfies ||r|| <= tol*||b|| */
  {
    ...
    status = (normr <= tol*normb) != 1;
    gsl_test(status, "%s toeplitz residual N="PCTZ"u a=%f b=%f c=%f normr=%.12e normb=%.12e",
...
test_random(const size_t N, const gsl_rng *r, const int compress)
{
  ...
  gsl_test(status, "%s random status s=%d N="PCTZ"u", desc, status, N);
  ...
  /* check that the residual satisfies ||r|| <= tol*||b|| */
  {
    ...
    status = (normr <= tol*normb) != 1;
    gsl_test(status, "%s random residual N="PCTZ"u normr=%.12e normb=%.12e",
             desc, N, normr, normb);
